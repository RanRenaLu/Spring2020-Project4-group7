...
library(ggplot2)
ggplot(mapping = aes(x= as.factor(test_knn[,3]), y = pred_knn_test) )+ geom_boxplot(aes(color =as.factor(test_knn[,3]))) + theme_light() + labs(title = "PMF Predicted Rating with Postprocessing v.s. True Value", x = "True Value", y = "Predicted") + theme(legend.title = element_blank(), plot.title = element_text(hjust = 0.5))
